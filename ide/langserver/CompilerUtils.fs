...
// to get blechmodule from  context
let getTCModuleFromTUP (ctx: CompilationUnit.Context<ImportChecking.ModuleInfo>) tup =
    let moduleInfo: ImportChecking.ModuleInfo =
        match ctx.loaded.TryGetValue (CompilationUnit.Implementation tup) with
        | true, Ok modInfoRes -> modInfoRes.info
        | _, Error _ -> failwithf "Found implementation for %s but it is errornous." <| tup.ToString()
...
// this function is to get program graph from compiler for given uri and file content
// let getGraphFromCompiler (uri: Uri) tcContext  blechModule     = 
//     let inputFile = uri.LocalPath
//     let logger = Diagnostics.Logger.create()   
//     // let result = getBlockGraphContext logger inputFile tcContext blechModule
//     let pgs = checkPackCausality logger tcContext blechModule
//     match pgs with
//     | Ok blockGraphContext ->
//         let graphStrings = 
//             blockGraphContext
//             |> Seq.map (fun kvp ->  kvp.Value.Graph) 
//             |> Seq.toList 
//         graphStrings 
...
let getGraphFromCompiler (uri: Uri) tcContext  blechModule     = 
    ...
    | Ok blockGraphContext ->
        ...
        // |> List.iter (fun Graph ->
        //     printfn "Graph:"
        //     let nodes = Graph.Nodes |> Seq.iter (fun node -> printfn "Node: %A" node )
        //     let edges = Graph.Edges |> Seq.iter (fun edge -> printfn "Edge: %A" edge)
        //     nodes
        //     edges
        // )
        let nodesAndEdges =
            graphStrings
            |> List.map (fun graph ->
                let nodes = graph.Nodes |> Seq.toList
...
// let runBlechCommand (uri: Uri) =
//     let inputFile = uri.LocalPath
//     let projectDir = System.IO.Path.GetDirectoryName inputFile
//     let inputFileName = System.IO.Path.GetFileNameWithoutExtension inputFile
//     let dotFileName = System.IO.Path.Combine(projectDir, sprintf "%s.dot" inputFileName)

//     // the path to the Blech compiler DLL
//     let blechCompilerPath = System.IO.Path.Combine(projectDir, "./src/blechc/bin/Debug/net6.0/blechc.dll")

//     // command for generating graph dot data 
//     let command = sprintf "-v d %s" (System.IO.Path.GetFileName inputFile)

//     // Set up process start info
//     let startInfo = ProcessStartInfo("blechc", command)
