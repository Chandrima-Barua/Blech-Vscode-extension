...
let parseDidShowedGraphParams (json: JsonValue): DidShowedGraphParams = 
    {
        textDocument = json?textDocument |> parseTextDocumentItem 
    }
...
let parseNotification (method: string) (maybeBody: option<JsonValue>): Notification = 
    ...
    | "textDocument/didshowGraph", Some json -> DidShowedGraph (parseDidShowedGraphParams json)
    | _, None -> 
...
let checkNull (json: option<JsonValue>): option<JsonValue> = 
    match json with 
    | Some JsonValue.Null -> None
    | None -> Some (JsonValue.String "1")
    | _ -> Some json.Value

let checkTrace (json: JsonValue): option<JsonValue> = 
    match json with 
    | JsonValue.Null -> None 
    | _ -> Some json 
...
let parseInitialize (json: JsonValue): InitializeParams = 
    { 
         processId = (json.TryGetProperty "processId") |> checkNull |> Option.map JsonExtensions.AsInteger
         rootUri = (json.TryGetProperty "rootUri") |> checkNull |> Option.map JsonExtensions.AsString |> Option.map Uri
         initializationOptions = json.TryGetProperty("initializationOptions") 
         capabilitiesMap = json?capabilities |> parseCapabilities 
         trace = json.TryGetProperty("trace") |> Option.bind checkTrace |> Option.map JsonExtensions.AsString |> Option.map parseTrace
    }
...
let parseDiagnostic (json: JsonValue): Diagnostic = 
    {
        range = json?range |> parseRange
        severity = json?severity |> JsonExtensions.AsString
        code = json.TryGetProperty("code") |> Option.map JsonExtensions.AsString 
        source = json?source.AsString()
        message = json?message.AsString()
        relatedInformation = [||] //json.TryGetProperty("relatedInformation") |> Option.map JsonExtensions.AsArray
    }
...
let parseShowGraphParams (json: JsonValue): ShowGraphParams = 
    {
        textDocument = json?textDocument |> parseTextDocumentIdentifier
    }
...
let parseRequest (method: string) (json: JsonValue): Request = 
    ...
    | "textDocument/showGraph" -> ShowGraph (parseShowGraphParams json)
